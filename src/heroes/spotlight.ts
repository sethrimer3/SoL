import type { AbilityBullet, CombatTarget, Player, Unit, Vector2D } from '../game-core';

type SpotlightHeroDeps = {
    Unit: typeof Unit;
...
export const createSpotlightHero = (deps: SpotlightHeroDeps) => {
    const { Unit, Vector2D, Constants, AbilityBullet } = deps;

    type SpotlightState = 'inactive' | 'setup' | 'active' | 'teardown';

    /**
     * Spotlight hero unit - reveals enemies in a narrow cone and fires rapid shots.
     */
    class Spotlight extends Unit {
        spotlightDirection: Vector2D | null = null;
        spotlightState: SpotlightState = 'inactive';
        spotlightStateElapsedSec: number = 0;
        spotlightFireCooldownSec: number = 0;
        spotlightLengthFactor: number = 0;
        spotlightRangePx: number = 0;

        constructor(position: Vector2D, owner: Player) {
            super(
                position,
                owner,
                Constants.SPOTLIGHT_MAX_HEALTH,
                Constants.SPOTLIGHT_ATTACK_RANGE,
                Constants.SPOTLIGHT_ATTACK_DAMAGE,
                Constants.SPOTLIGHT_ATTACK_SPEED,
                Constants.SPOTLIGHT_ABILITY_COOLDOWN
            );
            this.isHero = true;
        }
        ...
        updateSpotlightState(deltaTime: number): void {
            if (this.spotlightState === 'inactive') {
                return;
            }
        ...
        setSpotlightRangePx(rangePx: number): void {
            this.spotlightRangePx = rangePx;
        }

        canFireSpotlight(): boolean {
            return this.isSpotlightSetupComplete() && this.spotlightFireCooldownSec <= 0;
        }

        fireSpotlightAtTargets(targets: CombatTarget[], maxRangePx: number): AbilityBullet[] {
            const bullets: AbilityBullet[] = [];
            ...
            for (const target of targets) {
                ...
                const velocity = new Vector2D(
                    dx * invDistance * Constants.SPOTLIGHT_BULLET_SPEED,
                    dy * invDistance * Constants.SPOTLIGHT_BULLET_SPEED
                );
                ...
                const bullet = new AbilityBullet(
                    ...
                    velocity,
                    this.owner,
                    Constants.SPOTLIGHT_BULLET_DAMAGE
                );
