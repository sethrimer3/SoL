...
type MothershipHeroDeps = {
    Unit: typeof Unit;
    Vector2D: typeof Vector2D;
    Constants: typeof import('../constants');
    MuzzleFlash: typeof MuzzleFlash;
    BulletCasing: typeof BulletCasing;
    BouncingBullet: typeof BouncingBullet;
...
export const createMothershipHero = (deps: MothershipHeroDeps) => {
    ...
    class MiniMothership {
        ...
        constructor(position: Vector2D, initialVelocity: Vector2D, owner: Player) {
        ...
        update(deltaTime: number, targets: CombatTarget[]): void {
            // Update lifetime
        ...
        findNearestTarget(targets: CombatTarget[]): CombatTarget | null {
        ...
        checkCollision(boundary: number, asteroids: any[], buildings: any[]): boolean {
    ...
    class Mothership extends Unit {
        ...
        private miniMothershipsToSpawn: MiniMothership[] = [];

        constructor(position: Vector2D, owner: Player) {
            super(
                position,
                owner,
                Constants.MOTHERSHIP_MAX_HEALTH,
                Constants.MOTHERSHIP_ATTACK_RANGE,
                Constants.MOTHERSHIP_ATTACK_DAMAGE,
                Constants.MOTHERSHIP_ATTACK_SPEED,
        ...
        useAbility(direction: Vector2D): boolean {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const velocity = new Vector2D(
                    Math.cos(miniAngle) * speed,
                    Math.sin(miniAngle) * speed
                );

                // Create mini-mothership
                const mini = new MiniMothership(
         ...
         */
        getAndClearMiniMotherships(): MiniMothership[] {
