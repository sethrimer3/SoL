import type { Unit, Vector2D, Player, CombatTarget } from '../game-core';
...
export const createTankHero = (deps: TankHeroDeps) => {
    ...
    class CrescentWave {
        ...
        affectedUnits: Set<Unit> = new Set(); // Track which units have been stunned

        constructor(
            public position: Vector2D,
            public direction: Vector2D, // Normalized direction vector
            public angle: number, // Center angle of the wave
            public owner: Player
        ) {}
         ...
         */
        isPointInWave(point: Vector2D): boolean {
    ...
    class Tank extends Unit {
        private crescentWave: CrescentWave | null = null;

        constructor(position: Vector2D, owner: Player) {
            super(
                position,
                owner,
                Constants.TANK_MAX_HEALTH,
                Constants.TANK_ATTACK_RANGE,
                Constants.TANK_ATTACK_DAMAGE,
                Constants.TANK_ATTACK_SPEED,
                Constants.TANK_ABILITY_COOLDOWN,
        ...
        useAbility(direction: Vector2D): boolean {
            ...
            this.crescentWave = new CrescentWave(
         ...
         */
        getCrescentWave(): CrescentWave | null {
         ...
         */
        isPositionInShield(position: Vector2D): boolean {
