...
exports.createShadowHero = void 0;
const createShadowHero = (deps) => {
     ...
     */
    class ShadowDecoy {
        constructor(position, velocity, owner, heroMaxHealth) {
            this.opacity = 1.0; // Current opacity (1.0 = fully opaque, 0.75 = hit)
            this.targetOpacity = 1.0; // Target opacity to fade towards
            this.lifetime = 0;
         ...
         */
        checkCollision(boundary, asteroids, structures) {
    ...
    class Shadow extends Unit {
        constructor(position, owner) {
            super(position, owner, Constants.SHADOW_MAX_HEALTH, Constants.SHADOW_ATTACK_RANGE, Constants.SHADOW_ATTACK_DAMAGE, Constants.SHADOW_ATTACK_SPEED, Constants.SHADOW_ABILITY_COOLDOWN, Constants.SHADOW_COLLISION_RADIUS_PX);
            this.currentBeamTarget = null;
            this.beamDuration = 0; // How long we've been beaming current target
            this.decoyToSpawn = null;
            this.currentDamageMultiplier = 1.0; // For display
            this.isHero = true;
        }
        /**
        ...
        update(deltaTime, enemies, allUnits, asteroids = []) {
            ...
            this.currentDamageMultiplier = Math.min(Constants.SHADOW_BEAM_MAX_MULTIPLIER, 1.0 + (this.beamDuration * Constants.SHADOW_BEAM_MULTIPLIER_PER_SECOND));
            // Call parent update
        ...
        useAbility(direction) {
            ...
            const speed = Math.min(arrowLength * Constants.SHADOW_DECOY_SPEED_MULTIPLIER, Constants.SHADOW_DECOY_MAX_SPEED);
            // Normalize direction
            ...
            this.decoyToSpawn = new ShadowDecoy(new Vector2D(this.position.x, this.position.y), velocity, this.owner, this.maxHealth);
            return true;
        }
        /**
...
exports.createShadowHero = createShadowHero;
