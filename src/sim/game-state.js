...
class GameState {
    ...
    update(deltaTime) {
        ...
        for (const player of this.players) {
            ...
            if (player.stellarForge) {
                ...
                if (!this.isCountdownActive) {
                    ...
                    if (energyForMinions > 0) {
                        ...
                        const numStarlings = Math.min(Math.floor(player.stellarForge.incomingLightPerSec / Constants.FORGE_CRUNCH_ENERGY_PER_SEC_PER_STARLING), availableStarlingSlots);
                        ...
                        if (numStarlings > 0) {
                            for (let i = 0; i < numStarlings; i++) {
                                ...
                                const starling = new starling_1.Starling(spawnPosition, player, (_b = (_a = player.stellarForge) === null || _a === void 0 ? void 0 : _a.minionPath) !== null && _b !== void 0 ? _b : []);
            ...
            for (const mirror of player.solarMirrors) {
                ...
                const hasKnockback = Math.sqrt(Math.pow(mirror.knockbackVelocity.x, 2) + Math.pow(mirror.knockbackVelocity.y, 2)) > 0;
                if (this.checkCollision(mirror.position, 20, mirror)) {
                    ...
                    if (hasKnockback) {
                        ...
                        for (const asteroid of this.asteroids) {
                            const oldDist = Math.sqrt(Math.pow((oldMirrorPos.x - asteroid.position.x), 2) +
                                Math.pow((oldMirrorPos.y - asteroid.position.y), 2));
                            const newDist = Math.sqrt(Math.pow((mirror.position.x - asteroid.position.x), 2) +
                                Math.pow((mirror.position.y - asteroid.position.y), 2));
    ...
    updateAiMirrorPurchaseForPlayer(player) {
        ...
        const mirror = new solar_mirror_1.SolarMirror(spawnPosition, player);
    ...
    updateStateHash() {
        ...
        for (const particle of this.spaceDust) {
            ...
            mixString((_a = particle.impactColor) !== null && _a !== void 0 ? _a : '');
        }
        for (const player of this.players) {
            mix(player.energy);
            ...
            if (player.stellarForge) {
                ...
                mixString((_b = player.stellarForge.heroProductionUnitType) !== null && _b !== void 0 ? _b : '');
            ...
            for (const building of player.buildings) {
                ...
                if (building instanceof buildings_1.SubsidiaryFactory) {
                    ...
                    mixString((_c = building.currentProduction) !== null && _c !== void 0 ? _c : '');
    ...
    executeUnitTargetStructureCommand(player, data) {
        ...
        if (structureType === 'forge') {
            targetStructure = (_a = targetPlayer.stellarForge) !== null && _a !== void 0 ? _a : null;
        }
        else if (structureType === 'building') {
            targetStructure = (_b = targetPlayer.buildings[structureIndex]) !== null && _b !== void 0 ? _b : null;
        }
        else if (structureType === 'mirror') {
            targetStructure = (_c = targetPlayer.solarMirrors[structureIndex]) !== null && _c !== void 0 ? _c : null;
        }
        if (!targetStructure) {
            console.warn('Target structure not found for unit target command.');
            return;
        }
        for (const unitId of unitIds !== null && unitIds !== void 0 ? unitIds : []) {
            const unit = player.units.find(u => this.getUnitNetworkId(u) === unitId);
            if (unit) {
    ...
    updateDeathParticles(deltaTime) {
        ...
        for (const deathParticle of this.deathParticles) {
            ...
            const particleRadius = Math.max(0.75, (((_b = (_a = deathParticle.spriteFragment) === null || _a === void 0 ? void 0 : _a.width) !== null && _b !== void 0 ? _b : 2) * 0.5));
    ...
    executeUnitPathCommand(player, data) {
        ...
        const path = (waypoints !== null && waypoints !== void 0 ? waypoints : []).map((wp) => new math_1.Vector2D(wp.x, wp.y));
        for (const unitId of unitIds !== null && unitIds !== void 0 ? unitIds : []) {
            const unit = player.units.find(u => this.getUnitNetworkId(u) === unitId);
            if (unit) {
                unit.clearManualTarget();
    ...
    executeMirrorPurchaseCommand(player, data) {
        var _a, _b;
        const cost = typeof (data === null || data === void 0 ? void 0 : data.cost) === 'number' ? data.cost : Constants.STELLAR_FORGE_SOLAR_MIRROR_COST;
        const positionX = typeof (data === null || data === void 0 ? void 0 : data.positionX) === 'number' ? data.positionX : (_a = player.stellarForge) === null || _a === void 0 ? void 0 : _a.position.x;
        const positionY = typeof (data === null || data === void 0 ? void 0 : data.positionY) === 'number' ? data.positionY : (_b = player.stellarForge) === null || _b === void 0 ? void 0 : _b.position.y;
        ...
        player.solarMirrors.push(new solar_mirror_1.SolarMirror(new math_1.Vector2D(positionX, positionY), player));
    }
    ...
    executeMirrorMoveCommand(player, data) {
        ...
        const target = new math_1.Vector2D(targetX, targetY);
        for (const mirrorIndex of mirrorIndices !== null && mirrorIndices !== void 0 ? mirrorIndices : []) {
            const mirror = player.solarMirrors[mirrorIndex];
            if (mirror) {
                mirror.setTarget(target, this);
                if (typeof moveOrder === 'number') {
    ...
    executeMirrorLinkCommand(player, data) {
        ...
        let targetStructure = null;
        if (structureType === 'forge') {
            targetStructure = (_a = player.stellarForge) !== null && _a !== void 0 ? _a : null;
        }
        else if (structureType === 'building') {
            targetStructure = (_b = player.buildings[buildingIndex]) !== null && _b !== void 0 ? _b : null;
        }
        for (const mirrorIndex of mirrorIndices !== null && mirrorIndices !== void 0 ? mirrorIndices : []) {
            const mirror = player.solarMirrors[mirrorIndex];
            if (mirror) {
