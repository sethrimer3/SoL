...
class GameState {
            ...
            for (const player of this.players) {
                ...
                for (const mirror of player.solarMirrors) {
                    ...
                    if (bullet.checkHit(mirror)) {
                        ...
                        this.addDamageNumber(mirror.position, bullet.damage, Constants.MIRROR_MAX_HEALTH, mirror.health, mirrorKey);
                        bullet.lifetime = bullet.maxLifetime; // Mark for removal
                        break;
                    }
                }
            ...
            for (const player of this.players) {
                ...
                for (const mirror of player.solarMirrors) {
                    ...
                    if (projectile.checkHit(mirror)) {
                        ...
                        const mirrorKey = `mirror_${mirror.position.x}_${mirror.position.y}_${player.name}`;
                        this.addDamageNumber(mirror.position, projectile.damage, Constants.MIRROR_MAX_HEALTH, mirror.health, mirrorKey);
                        hasHit = true;
                        break;
                    }
                }
            ...
            for (const player of this.players) {
                ...
                for (const mirror of player.solarMirrors) {
                    if (laser.checkHit(mirror)) {
                        ...
                        this.damageNumbers.push(new damage_number_1.DamageNumber(mirror.position, laser.damage, this.gameTime));
                    }
                }
                // Check stellar forge
    ...
    updateAiMirrorsForPlayer(player) {
        ...
        for (let i = 0; i < mirrorCount; i++) {
            ...
            const target = new math_1.Vector2D(sun.position.x + Math.cos(angleRad) * radiusPx, sun.position.y + Math.sin(angleRad) * radiusPx);
    ...
    initializeMirrorMovement() {
        ...
        for (const player of this.players) {
            ...
            if (player.solarMirrors.length >= 1) {
                const leftTarget = new math_1.Vector2D(forgePos.x + Math.cos(leftAngle) * Constants.MIRROR_COUNTDOWN_DEPLOY_DISTANCE, forgePos.y + Math.sin(leftAngle) * Constants.MIRROR_COUNTDOWN_DEPLOY_DISTANCE);
                player.solarMirrors[0].setTarget(leftTarget);
            }
            if (player.solarMirrors.length >= 2) {
                const rightTarget = new math_1.Vector2D(forgePos.x + Math.cos(rightAngle) * Constants.MIRROR_COUNTDOWN_DEPLOY_DISTANCE, forgePos.y + Math.sin(rightAngle) * Constants.MIRROR_COUNTDOWN_DEPLOY_DISTANCE);
    ...
    createDeathParticles(entity, color) {
        ...
        let baseSize;
        const rng = (0, seeded_random_1.getGameRNG)();
        if (entity instanceof buildings_1.Building) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const particle = new particles_1.DeathParticle(new math_1.Vector2D(entity.position.x, entity.position.y), velocity, rng.nextAngle(), // Random initial rotation
            fragment, fadeStartTime);
