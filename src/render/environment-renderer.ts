...
export class EnvironmentRenderer {
    ...
    /**
     * Draw experimental field atmospheric effects – nebulae, ribbons, caustics,
     * sun halos, glints, and vignettes.  Previously lived inside GameRenderer;
     * extracted here because the logic is purely environmental/visual and
     * depends only on the renderer context, game suns, and noise utilities.
     */
    public drawExperimentalFieldAtmospherics(
        ...
        const { ctx, camera, zoom, graphicsQuality, worldToScreen } = context;
        ...
        const qualityIntensity = isUltraQuality ? 1 : isHighQuality ? 0.82 : graphicsQuality === 'medium' ? 0.66 : 0.52;
        const glintCount = isUltraQuality ? 32 : isHighQuality ? 24 : graphicsQuality === 'medium' ? 14 : 8;
        const vignetteAlpha = isUltraQuality ? 0.16 : isHighQuality ? 0.13 : graphicsQuality === 'medium' ? 0.1 : 0.08;
        const causticLayerCount = isUltraQuality ? 3 : isHighQuality ? 2 : 1;
        const causticAlphaBase = isUltraQuality ? 0.07 : isHighQuality ? 0.055 : graphicsQuality === 'medium' ? 0.045 : 0.035;
        const edgeBloomAlpha = isUltraQuality ? 0.115 : isHighQuality ? 0.09 : graphicsQuality === 'medium' ? 0.072 : 0.058;
        ...
        const cameraNoiseWorldX = camera.x * 0.0012;
        const cameraNoiseWorldY = camera.y * 0.0012;
        ...
        for (const sun of game.suns) {
            ...
            const haloRadiusPx = Math.max(180, sun.radius * zoom * (4 + qualityIntensity * 2.4));
