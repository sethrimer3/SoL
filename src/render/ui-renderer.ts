...
import { getInGameMenuLayout, getGraphicsMenuMaxScroll, InGameMenuTab, RenderLayerKey } from './in-game-menu';
...
export class UIRenderer {
    ...
    private drawHeroHourglass(centerX: number, centerY: number, radius: number, context: UIRendererContext): void {
    ...
    private drawHeroCheckmark(centerX: number, centerY: number, radius: number, context: UIRendererContext): void {
        const iconWidth = radius * 0.7;
    ...
    private isRenderLayerEnabled(layer: RenderLayerKey, context: UIRendererContext): boolean {
        switch (layer) {
            case 'suns':
